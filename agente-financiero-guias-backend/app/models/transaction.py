...
class Transaction(Base):
    ...
    payment_method_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("payment_methods.id"), nullable=True)
    type: Mapped[TransactionType] = mapped_column(SQLEnum(TransactionType, name="transactiontype"), nullable=False)
    is_budgeted: Mapped[bool] = mapped_column(Boolean, default=False)
    expense_origin: Mapped[ExpenseOrigin] = mapped_column(SQLEnum(ExpenseOrigin, name="expenseorigin"), nullable=True)
    ...
    budget = relationship("ExpenseBudget", foreign_keys=[budget_id], overlaps="transaction")
    income_budget = relationship("IncomeBudget", foreign_keys=[income_budget_id], overlaps="transaction")
    ...
    payment_method_ref = relationship("PaymentMethod", back_populates="transactions")
