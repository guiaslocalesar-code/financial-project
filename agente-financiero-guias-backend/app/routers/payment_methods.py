...
async def get_payment_method(pm_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def update_payment_method(pm_id: UUID, pm_in: PaymentMethodUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(PaymentMethod).where(PaymentMethod.id == pm_id))
    payment_method = result.scalar_one_or_none()
    if not payment_method:
        raise HTTPException(status_code=404, detail="Payment method not found")
...
@router.delete("/{pm_id}")
async def delete_payment_method(pm_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(PaymentMethod).where(PaymentMethod.id == pm_id))
    payment_method = result.scalar_one_or_none()
    if not payment_method:
        raise HTTPException(status_code=404, detail="Payment method not found")
