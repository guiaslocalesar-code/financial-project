...
def migrar_tabla_condicional(src_conn, tabla, where_clause, delete_keys=None, only_keys=None):
    ...
    with src_conn.cursor(name=f"cur_{tabla}_2026") as cur:
        cur.execute(f"SELECT {cols_sql} FROM public.{tabla} WHERE {where_clause} ORDER BY created_at;")
...
def main():
    print("=" * 60)
    print("SINC DEL AÑO 2026: CLOUD SQL (GCP) -> SUPABASE via REST")
    print("=" * 60)
    
    src = connect_src()
    
    # 1. Migrar income_budgets del 2026 SIN transaction_id para evitar FK violation (ya que las transactions no existen aun)
    migrar_tabla_condicional(src, "income_budgets", "period_year >= 2026", delete_keys=['transaction_id'])
    
    # 2. Migrar transactions asociadas (esto usa solo de los keys validos en supabase, perdiendo pago_metodo_id si no existe)
    migrar_tabla_condicional(src, "transactions", "type = 'INCOME' AND transaction_date >= '2026-01-01'")
    
    # 3. Actualizar income_budgets CON su transaction_id ahora que las transacciones existen
    migrar_tabla_condicional(src, "income_budgets", "period_year >= 2026")
