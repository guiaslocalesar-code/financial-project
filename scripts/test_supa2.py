...
def main():
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(f"SELECT {cols_sql} FROM public.income_budgets WHERE period_year >= 2026 LIMIT 1;")
        row = cur.fetchone()
    if row:
        payload = [serialize_row(dict(zip(cols, row)))]
        r = requests.post(f"{SUPABASE_URL}/rest/v1/income_budgets", headers=supabase_headers(), data=json.dumps(payload))
        print("income_budgets:", r.status_code, r.text)
    ...
    if row:
        ...
        print("transactions:", r.status_code, r.text)
